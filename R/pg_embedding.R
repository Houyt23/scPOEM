...
#' gg_net <- GGN(example_data_single$Y, file.path(dirpath, "single"), 1, 5, T)
...
pg_embedding <- function(gg_net, pp_net, pg_net_list, dirpath, relearn_pg_embedding=T, d=100, numwalks=5, walklength=3, epochs=100, neg_sample=5, batch_size=32, weighted = TRUE, exclude_pos=FALSE, seed=0, python_env = "scPOEM_env") {
  ...
  E_result <- metapath2vec_pg(gg_net, pp_net, pg_net_list, dirpath, as.integer(d), as.integer(numwalks), as.integer(walklength), as.integer(epochs), as.integer(neg_sample), as.integer(batch_size), weighted, exclude_pos, as.integer(seed))
  toc()
  E <- E_result[[1]]
...
pg_embedding_use <- function(gg_net, pp_net, pg_net_list, dirpath, relearn_pg_embedding=T, d=100, numwalks=5, walklength=3, epochs=100, neg_sample=5, batch_size=32, weighted = TRUE, exclude_pos=FALSE, seed=0) {
  if (!relearn_pg_embedding) {
    cat("Load embedding results\n")
    E <- readMM(file.path(dirpath, "embedding/node_embeddings.mtx"))
    peak_node <- read.csv(file.path(dirpath, "embedding/node_used_peak.csv"), header = FALSE)[[1]]
