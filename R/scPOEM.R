...
scPOEM <- function(mode = c("single", "compare"), input_data, dirpath, count_device=1, nComp=5, seed=0, numwalks=5, walklength=3, epochs=100, neg_sample=5, batch_size=32, weighted = TRUE, exclude_pos=FALSE, d=100, rebuild_GGN=T, rebuild_PPN=T, rebuild_PGN_Lasso=T, rebuild_PGN_RF=T, rebuild_PGN_XGB=T, relearn_pg_embedding=T, pg_method=c("Lasso", "RF", "XGBoost"), python_env = "scPOEM_env") {
  ...
  if (any(missing)) {
    ...
    stop("Missing Python dependencies.")
  }
  ...
  if (mode == "single") {
    ...
    gg_net <- GGN_use(input_data$Y, dirpath, count_device, nComp, rebuild_GGN)
    ...
    if ("RF" %in% pg_method){
      net_RF <- PGN_RF_use(input_data$X, input_data$Y, input_data$gene_data, input_data$neibor_peak, dirpath, count_device, rebuild_PGN_RF, seed=seed)
    ...
    single_result <- pg_embedding_use(gg_net, pp_net, pg_net_list, dirpath, d=d, numwalks=numwalks, walklength=walklength, epochs=epochs, neg_sample=neg_sample, batch_size=batch_size, weighted=weighted, exclude_pos=exclude_pos, seed=seed)
    ...
    for (state_name in state_names) {
      ...
      cat("Construct gene-gene net.\n")
      gg_net <- GGN_use(state_data$Y, dirpath_s, count_device, nComp, rebuild_GGN)
      cat("Construct peak-peak net.\n")
      ...
      cat("Construct peak-gene net via random forest.\n")
      if ("RF" %in% pg_method){
        net_RF <- PGN_RF_use(state_data$X, state_data$Y, state_data$gene_data, state_data$neibor_peak, dirpath_s, count_device, rebuild_PGN_RF, seed=seed)
        pg_net_list <- c(pg_net_list, list(net_RF))
      }
      cat("Construct peak-gene net via XGBoost.\n")
      if ("XGBoost" %in% pg_method){
      ...
      cat("Learn low-dimensional representations for peaks and genes.\n")
      compare_result[[state_name]] <- pg_embedding_use(gg_net, pp_net, pg_net_list, dirpath_s, relearn_pg_embedding=relearn_pg_embedding, d=d, numwalks=numwalks, walklength=walklength, epochs=epochs, neg_sample=neg_sample, batch_size=batch_size, weighted=weighted, exclude_pos=exclude_pos, seed=seed)
    }
