...
PPN <-function(X, peak_data, cell_data, genome, dirpath, rebuild_PPN=T, seed=0){
  ...
  if (!requireNamespace("monocle3", quietly = TRUE)) {
    stop("The 'monocle3' package is required. Please install it with:\n",
  ...
  if (!requireNamespace("cicero", quietly = TRUE)) {
    stop("The 'cicero' package is required. Please install it with:\n",
         "if (!requireNamespace('BiocManager', quietly = TRUE)) install.packages('BiocManager')\n",
  ...
  use_cicero <- function(indata, cellinfo, peakinfo, genome, dirpath){
    ...
    input_cds <- input_cds[rowSums(monocle3::exprs(input_cds)) != 0,]
    ...
    input_cds <- monocle3::reduce_dimension(input_cds, reduction_method = 'UMAP',
                                  preprocess_method = "LSI")
    monocle3::plot_cells(input_cds)

    #access the UMAP coordinates from the input CDS object
    umap_coords <- reducedDim(input_cds)

    #run make_cicero_cds
    cicero_cds <- cicero::make_cicero_cds(input_cds, reduced_coordinates = umap_coords)
    gc()

    #genome <- fread(genome_file, header = FALSE)#human.hg38.genome,mouse.mm10.genome
    conns <- cicero::run_cicero(cicero_cds, genome, sample_num = 100)
    head(conns)
