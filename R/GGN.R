...
#' # Construct GGN net.
#' gg_net <- GGN(example_data_single$Y, file.path(dirpath, "single"))
#'}
#'
#' @export
GGN <- function(Y, dirpath, count_device=1, nComp=5, rebuild_GGN=T, python_env = "scPOEM_env") {
  tryCatch({
...
GGN_use <- function(Y, dirpath, count_device, nComp=5, rebuild_GGN=T) {
  if (!rebuild_GGN){
    cat("Load gene-gene network\n")
    gg_net <- readMM(file.path(dirpath, "test/GGN.mtx"))
    cat("\n")
