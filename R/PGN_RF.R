...
PGN_RF <- function(X, Y, gene_data, neibor_peak, dirpath, count_device=1, rebuild_PGN_RF=T, seed=0, python_env = "scPOEM_env") {
  if (!rebuild_PGN_RF){
...
PGN_RF_use <- function(X, Y, gene_data, neibor_peak, dirpath, count_device=1, rebuild_PGN_RF=T, seed=0) {
  if (!rebuild_PGN_RF){
    cat("Load peak-gene network constructed by random forest\n")
    net_RF <- readMM(file.path(dirpath, "test/PGN_RF.mtx"))
    cat("\n")
