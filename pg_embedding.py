...
def metapath2vec_pg(dirpath):

    peak_net = sparse.coo_matrix(mmread(os.path.join(dirpath, "test/PPN.mtx")))
    peak_net = np.abs(peak_net)
    ...
    np.savez(os.path.join(dirpath, "test/node_used.npz"), used_peaks)
    len_peak = pg_net.shape[0]
    len_gene = pg_net.shape[1]
    pg_net = pg_net.tocsr()[used_peaks, :].tocoo()
    gp_net = gp_net.tocsr()[used_peaks, :].tocoo()
    len_peak = pg_net.shape[0]
    len_gene = pg_net.shape[1]
	
    peak_net = peak_net.tocsr()[used_peaks, :][:, used_peaks].tocoo()
    clip_min = np.percentile(peak_net.data, 10)
    ...
    gene_net = sparse.coo_matrix(gene_net)
    clip_min = np.percentile(gene_net.data, 10)
    clip_max = np.percentile(gene_net.data, 90)
    gene_net.data = np.clip(gene_net.data, clip_min, clip_max)
